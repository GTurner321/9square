...
const FractionAddDiag = (() => {
  ...
  function buildFractionAddSVG(params, opts = {}) {
    ...
    // Each fraction's own label, below its own circle.
    const labelY = radius + 24;
    body += textEl(cxA, labelY, `${aNum}/${aDen}`, 'middle', fontSize, 700);
    extend(...textBoundsBox(cxA, labelY, 'middle', `${aNum}/${aDen}`, fontSize));
    body += textEl(cxB, labelY, `${bNum}/${bDen}`, 'middle', fontSize, 700);
    extend(...textBoundsBox(cxB, labelY, 'middle', `${bNum}/${bDen}`, fontSize));
